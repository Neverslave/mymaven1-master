...
function addressinit(){
    var province = ChinaCity.getProvince();
 ...
 function changeCity(ss) {
        var  city =ChinaCity.getCity(ss);
        var  ct= document.getElementById('city');
        with(ct) {
            for (let i = 0; i < city.length; i++) {
                options[i] = new Option(city[i], city[i]);

            }
        }
 }
 function  changeArea(pv,ct){
        var area =  ChinaCity.getArea(pv,ct);
        var ar = document.getElementById('town')
     with(ar) {
         for (let i = 0; i <area.length ; i++) {
             options[i] = new Option(area[i], area[i]);
         }
